...
import {
  ...
  XCircle, Users, Shield, Fingerprint
} from 'lucide-react'
...
export default function SettingsPanel({ user }: SettingsPanelProps) {
  ...
  const handleToggleNotifications = async () => {
    if (notifications) {
      await unsubscribe(); setNotifications(false)
      localStorage.setItem('notifications', 'false')
    } else {
      const ok = await subscribe(); setNotifications(ok)
  ...
  const fetchProfile = async () => {
    ...
    if (profile) {
      ...
      if (profile.role === 'landlord') {
        ...
        const { data } = await supabase
          .from('account_deletion_requests').select('*')
  ...
  const handleChangePassword = async () => {
    ...
    if (!strong.test(newPassword)) { showFeedback('Password must be 8+ characters with a number and special character.', true); return }
  ...
  const handleRequestDeletion = async () => {
    ...
    try {
      const { error } = await supabase.from('account_deletion_requests')
        .insert({ user_id: user!.id, reason: deleteReason.trim(), status: 'pending' })
      if (error) throw error
  ...
  const handleCancelRequest = async () => {
    ...
    await supabase.from('account_deletion_requests').delete().eq('id', myDeletionRequest.id)
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'pending':  return { icon: <Clock className="w-3.5 h-3.5" />,       label: 'Pending Review', style: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/20 dark:border-amber-800 dark:text-amber-400' }
      case 'approved': return { icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: 'Approved',       style: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/20 dark:border-emerald-800 dark:text-emerald-400' }
      case 'rejected': return { icon: <XCircle className="w-3.5 h-3.5" />,      label: 'Rejected',       style: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950/20 dark:border-red-800 dark:text-red-400' }
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-background">
      <div className="p-5 sm:p-8 space-y-4 max-w-2xl mx-auto w-full">
        ...
        <Section>
          <div className="flex items-center gap-4">
            <div className="relative shrink-0">
              <div className="w-16 h-16 rounded-2xl bg-accent/10 border-2 border-accent/20 flex items-center justify-center overflow-hidden">
                {avatarUrl
                  ...
                  : <span className="text-2xl font-bold text-accent">{fullName.charAt(0).toUpperCase() || '?'}</span>
                }
              </div>
              <button
            ...
            <div className="flex-1 min-w-0">
              <p className="font-bold text-foreground text-lg truncate">{fullName || 'No name set'}</p>
        ...
        {/* Update name */}
        <Section>
          <SectionHeader icon={<UserIcon className="w-4 h-4 text-accent" />} title="Update Full Name" />
          <div className="space-y-3">
            ...
            />
            <Button
              onClick={handleUpdateProfile} disabled={isSavingProfile}
        ...
        {/* Change password */}
        <Section>
          <SectionHeader icon={<Lock className="w-4 h-4 text-accent" />} title="Change Password" />
          <div className="space-y-3">
            ...
            <Input
              ...
              className="bg-secondary border-border text-foreground rounded-xl h-11"
            />
            <p className="text-xs text-muted-foreground">Min 8 characters with a number and special character</p>
            <Button
              onClick={handleChangePassword} disabled={isSavingPassword || !newPassword}
              className="w-full bg-accent hover:bg-accent/90 text-white rounded-xl h-11 shadow-sm shadow-accent/20"
            >
        ...
        {/* Preferences */}
        <Section>
          <SectionHeader icon={<Fingerprint className="w-4 h-4 text-accent" />} title="Preferences" />
          <div className="space-y-3">
            {[
              {
              ...
              {
                icon: isDark ? <Moon className="w-4 h-4 text-accent" /> : <Sun className="w-4 h-4 text-accent" />,
              ...
              <div key={item.title} className="flex items-center justify-between p-3.5 bg-secondary rounded-xl">
                ...
                >
                  <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${item.checked ? 'translate-x-5' : 'translate-x-0'}`} />
                </button>
              </div>
            ))}
          </div>
                ...
                <div key={req.id} className="p-4 bg-secondary rounded-xl border border-border space-y-3">
                  ...
                  {showConfirmDelete === req.id && (
                    <div className="bg-destructive/8 border border-destructive/25 rounded-xl p-4 space-y-3">
                      <p className="text-sm font-semibold text-destructive">⚠️ This action is irreversible</p>
                      <p className="text-xs text-muted-foreground">
                        All data for <span className="font-semibold text-foreground">{req.profiles?.full_name}</span> will be permanently deleted including messages, complaints, requests and profile.
                      </p>
                      <div className="flex gap-2">
                        <Button
        ...
        {role === 'tenant' && (
          <div className="bg-card border border-destructive/25 rounded-2xl p-5">
            ...
            {!myDeletionRequest ? (
              ...
              <div className="space-y-3">
                <div className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl border text-sm font-medium ${getStatusConfig(myDeletionRequest.status).style}`}>
                ...
                {myDeletionRequest.status === 'pending' && (
                  <p className="text-xs text-muted-foreground">Your request is awaiting landlord review.</p>
                )}
                {myDeletionRequest.status === 'rejected' && (
                  <p className="text-xs text-muted-foreground">Your deletion request was rejected. Contact your landlord for more information.</p>
                )}
                {myDeletionRequest.reason && (
                  <div className="text-xs text-muted-foreground bg-secondary rounded-xl p-3">
                    <span className="font-semibold">Your reason: </span>{myDeletionRequest.reason}
                  </div>
                )}
                {myDeletionRequest.status === 'pending' && (
                  <Button onClick={handleCancelRequest} variant="outline" className="w-full border-border h-10 text-xs rounded-xl">
                    Cancel Request
                  </Button>
                )}
              </div>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  onClick={handleRequestDeletion} disabled={isSubmittingDelete}
